...
- (void)reloadData {
    if (self.dataSource) {
        ...
        for (int i = 0; i < numberOfSlots; i++) {
            ...
            NSInteger scrollLayerTopIndex = - (i + self.minNoOfRotations + 3) * iconCount;
    ...
    if (isSliding) {
        ...
        [CATransaction setCompletionBlock:^{
            ...
            for (int i = 0; i < [_slotScrollLayerArray count]; i++) {
                ...
                for (int j = 0; j < slotIconsCount * (self.minNoOfRotations + i) + resultIndex - currentIndex; j++) {
                ...
                for (CALayer *toBeDeletedLayer in toBeDeletedLayerArray) {
                    ...
                    CGFloat shiftY = slotIconsCount * toBeAddedLayer.frame.size.height * (self.minNoOfRotations + i + 3);
        ...
        for (int i = 0; i < [_slotScrollLayerArray count]; i++) {
            ...
            NSUInteger howManyUnit = (i + self.minNoOfRotations) * slotIconsCount + resultIndex - currentIndex;
            NSInteger durationFactor = [self.dataSource slotDurationFactorinSlotMachine:self];
            ...
            slideAnimation.duration = self.singleUnitDuration *durationFactor;
